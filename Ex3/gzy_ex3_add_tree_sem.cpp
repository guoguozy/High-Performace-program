...
void *add_tree(void *node)
{
    ...
    if ((thread_rank % step) == 0)
    {
        while (step < n)
        {
            cube[t->i][thread_rank * (n / (num_threads * 2))][t->j] += cube[t->i][thread_rank * (n / (num_threads * 2)) + step][t->j];

            pthread_mutex_lock(&mutex1);
            counter1++;
            pthread_mutex_unlock(&mutex1);
            while (counter1 < num_threads / step)
                ;
            counter1 = 0;
            step *= 2;
        }
...
int main(int arg_1, char **arg_2)
{
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
            for (int k = 0; k < n; k += (n / (num_threads * 2)))
                for (int h = 1; h < n / (num_threads * 2); ++h)
                {
                    if (k + h == n)
                        break;
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
        {
            ...
            {
                Block node = Block(i, k, j);
                pthread_create(&threads_for_add_tree[k], NULL, add_tree, (void *)&node);
            }
